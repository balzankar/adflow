...
<?php
 if(isset($_POST['add_search'])){
   ...
   if(empty($errors)){
      ...
      $sql .= " VALUES ('{$search_name}','{$user_name}','Amazon India')";
      if($db->query($sql)){
        ...
        <div class="panel-body">
          <form method="post" action="search.php">
            <div class="form-group">
                <input type="text" class="form-control" name="search-name" placeholder="Search Amazon India">              
          ...
          <form method="post" action="search.php">
            <div class="form-group">
                <select  type="text" class="form-control" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);" name="search-name" placeholder="retailers">
                    ...
                    <option value="search2.php">Flipkart India</option>
                </select>        
            </div>
        </form>
        </div>
